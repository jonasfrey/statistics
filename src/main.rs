...
#[wasm_bindgen]
pub fn f_n_max__for_u8(a_n: &[u8]) -> Result<u8, JsValue> {
    a_n.iter()
        .max()
        .cloned()
        .ok_or_else(|| JsValue::from_str("Array is empty"))
}
#[wasm_bindgen]
...
// }
#[wasm_bindgen]
struct O_n_mean_n_min_n_max_for_u8 {
    n_mean: f64,
    n_min: u8,
    n_max: u8,
}
#[wasm_bindgen]
pub fn f_a_n_mean_n_sum_n_min_n_max__for_u8(a_n: &[u8]) -> Vec<f64> {
    ...
    vec![
        o_result.n_mean, 
        n_sum, 
        o_result.n_min as f64,
        o_result.n_max as f64
    ]
}
