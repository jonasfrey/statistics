...
#[wasm_bindgen]
pub fn f_a_o_n_mean_n_sum__for_u8(a_n: &[u8], n_channels: usize) -> Vec<O_n_mean_n_sum__for_u8> {
    let n_len = a_n.len() as f64 / n_channels as f64;
    let a_n_t__init = vec![0.0; n_channels];
    let a_n_t__result = a_n
        .iter()
        .enumerate()
        .fold(a_n_t__init, |mut a_n_acc, (n_idx, &val)| {
    ...
    for n_channel in 0..n_channels {
        ...
        let n_sum = n_mean * n_len;
        a_o.push(O_n_mean_n_sum__for_u8 {
            n_mean,
            // n_mean_nor,
            n_sum,
        });
...
#[wasm_bindgen]
pub fn f_a_o_n_min_n_max__for_u8(a_n: &[u8], n_channels: usize) -> Vec<O_n_min_n_max__for_u8> {
    let n_len = a_n.len() as f64 / n_channels as f64;
    ...
    let mut a_o = Vec::new();
    for n_channel in 0..n_channels {
        a_o.push(O_n_min_n_max__for_u8 {
...
pub fn f_a_o_mean_n_sum_n_min_n_max__for_u8(
    ...
    let a_o_result = a_n
        .iter()
        .enumerate()
        .fold(a_o_init, |mut a_o_acc, (n_idx, &val)| {
            let n_idx_channel = n_idx % n_channels;
    ...
    for n_channel in 0..n_channels {
        ...
        a_o.push(O_n_mean_n_sum_n_min_n_max_for_u8 {
            n_mean: o_result.n_mean,
...
#[wasm_bindgen]
pub fn f_a_n_u8__sorted(a_n: &[u8]) -> Vec<u8> {
    let mut a_n_sorted = a_n.to_vec();
    a_n_sorted.sort();
    a_n_sorted
}
#[wasm_bindgen]
pub fn f_a_n_u8__sorted_unstable(a_n: &[u8]) -> Vec<u8> {
    let mut a_n_sorted = a_n.to_vec();
...
////    }).join('\n')
#[wasm_bindgen]
pub fn f_a_n_f32__sorted(a_n: &[f32]) -> Vec<f32> {
    let mut a_n_sorted = a_n.to_vec();
...
pub fn f_a_n_f64_normalized__from_a_n_u64__loop(
    ...
    for &n_value in a_n_u64 {
        a_n_u64__normalized.push(((n_value as f64 - n_min as f64) / n_range as f64))
    }
    a_n_u64__normalized
}
...
pub fn f_a_n_f64_normalized__from_a_n_u64__iermapcollect(
    ...
    a_n_u64
        .iter()
        .map(|&n_value| ((n_value as f64 - n_min as f64) / n_range as f64))
        .collect()
}
